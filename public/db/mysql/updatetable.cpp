...
int main(int argc,char *argv[])
{
    ...
    // 另一种修改表的方式
    stmt.prepare("update girls set name='xxx',weight=27.58,btime=now() where id in (1,3)");
    stmt.execute();
    ...
    */
    ...
    for (int i = 0; i < 5; i++) {
        ...
        // 为结构体变量的成员赋值
        stgirls.id = i;                                     // 超女编号
        sprintf(stgirls.name, "%50d cute girl", i + 1);     // 超女姓名
        stgirls.weight = i + 20;                            // 超女体重
        sprintf(stgirls.btime, "2022-06-03 10:30:%02d", i); // 报名时间
