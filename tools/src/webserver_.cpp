/*
 * 程序名：webserver_.cpp，此程序是数据服务总线的服务端程序，采用了长连接。
 *
 */
#include "_public.h"
#include "_ooci.h"
...
int main(int argc, char *argv[])
{
  ...
  // 如果采用了黑名单或者白名单方法，把黑名单和白名单用参数表管理起来，
  // 服务程序启动的时候，把表中的参数加载到容器中，服务程序在运行的过程中，
  // 专门用一个线程定期的重新加载参数，每隔几分钟加载一次就行了。
...
void _help(char *argv[])
{
  printf("Using:/project/tools1/bin/webserver_ logfilename xmlbuffer\n\n");

  printf("Sample:/project/tools1/bin/procctl 10 /project/tools1/bin/webserver_ /log/idc/webserver_.log \"<connstr>scott/tiger@snorcl11g_132</connstr><charset>Simplified Chinese_China.AL32UTF8</charset><port>8080</port>\"\n\n");
